...
export function EditBookingDialog({
  ...
  const [semesters, setSemesters] =
    useState<SemesterSetting[]>(semesterSettings);
  ...
  async function onSubmit(values: z.infer<typeof bookingFormSchema>) {
    ...
    if (!isAdmin) {
      ...
      if (
        ...
        isDateInLockedPeriod(startDateTime, semesters, false)
      ) {
    ...
    if (
      ...
      const isInSemester = semesters.some((semester) =>
        isDateInSemester(startDateTime, semester),
      );
              ...
              <div className="grid gap-6 lg:grid-cols-2">
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-amber-800 dark:text-amber-300 mt-1">
                            ...
                            {format(new Date(nextSemester.start_date), "MM/dd")}{" "}
                            - {format(new Date(nextSemester.end_date), "MM/dd")}
                            ) 暫不開放預約，請等待管理員開放。
                          </p>
                        </div>
                      </div>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>
                          {rooms.find((r) => r.id === field.value)?.room_code}{" "}
                          {rooms.find((r) => r.id === field.value)?.room_type &&
                            `(${rooms.find((r) => r.id === field.value)?.room_type})`}
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                ...
                <div className="space-y-4">
                  <div className="text-sm font-medium">
                    空間預約狀況 -{" "}
                    {rooms.find((r) => r.id === form.watch("roomId"))?.name ||
                      "選擇空間"}
                  </div>
                  {form.watch("roomId") ? (
                    <RoomTimetable
