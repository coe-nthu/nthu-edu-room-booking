...
const rooms = [
  room({ id: "third", name: "Third Floor Studio", room_code: "301", floor: "3F", capacity: 60, room_type: "Studio" }),
  room({ id: "basement", name: "Basement Room", room_code: "B101", floor: "B1F", capacity: 20, room_type: "Meeting" }),
  room({ id: "first", name: "First Floor Hall", room_code: "101", floor: "1F", capacity: 45, room_type: "Classroom" }),
]
...
describe("SpaceList", () => {
  ...
  it("filters rooms by search text and shows an empty state", async () => {
    ...
    await user.type(screen.getByPlaceholderText("搜尋空間名稱、代號..."), "studio")

    await waitFor(() => {
      expect(screen.getByRole("heading", { name: "Third Floor Studio" })).toBeTruthy()
      expect(screen.queryByRole("heading", { name: "Basement Room" })).toBeNull()
    })

    await user.clear(screen.getByPlaceholderText("搜尋空間名稱、代號..."))
    await user.type(screen.getByPlaceholderText("搜尋空間名稱、代號..."), "no match")
