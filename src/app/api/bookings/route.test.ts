...
describe("POST /api/bookings", () => {
  ...
  it("rejects invalid request bodies", async () => {
    ...
    await expect(responseJson(response)).resolves.toEqual({ error: "無效的資料格式" })
  })
  ...
  it("rejects non-admin access to admin-only rooms", async () => {
    ...
    await expect(responseJson(response)).resolves.toEqual({ error: "此空間僅限管理員借用" })
  })
  ...
  it("rejects overlapping bookings", async () => {
    ...
    await expect(responseJson(response)).resolves.toEqual({ error: "該時段已被預約" })
  })
