...
async function getRelatedBookings(
  bookingId: string,
  ...
  if (!booking.recurrence_group_id) return [booking];

  const { data: relatedBookings } = await client
    .from("bookings")
    .select("id, room_id, start_time, end_time, recurrence_group_id")
    .eq("recurrence_group_id", booking.recurrence_group_id);

  return relatedBookings || [booking];
}

async function ensureNoApprovedConflicts(
  bookings: {
    room_id: string;
    start_time: string;
    end_time: string;
  ...
  for (const booking of bookings) {
    const { data: overlaps } = await client
      .from("bookings")
    ...
    const conflictingBookings = (overlaps || []).filter(
      (overlap) =>
        !recurrenceGroupId || overlap.recurrence_group_id !== recurrenceGroupId,
    );
...
export async function getRoomApprovers(
  roomId: string,
): Promise<RoomApprover[]> {
...
export async function createApprovalStepsForBooking(
  bookingId: string,
  roomId: string,
) {
  const supabaseAdmin = createServiceClient();
...
export async function approveStep(bookingId: string, stepId: string) {
  ...
  const allPrevApproved =
    !prevSteps ||
    prevSteps.every((s) => s.status === "approved" || s.status === "skipped");
  ...
  if (allApproved) {
    // All steps approved → check for conflicts and approve the whole logical request.
    const relatedBookings = await getRelatedBookings(bookingId, supabaseAdmin);
    await ensureNoApprovedConflicts(relatedBookings, supabaseAdmin);

    const recurrenceGroupId = relatedBookings[0]?.recurrence_group_id;
    const updateQuery = supabaseAdmin
      .from("bookings")
      .update({ status: "approved" });

    if (recurrenceGroupId) {
      await updateQuery.eq("recurrence_group_id", recurrenceGroupId);
    } else {
...
export async function rejectStep(
  ...
  // Reject the entire logical request
  const relatedBookings = await getRelatedBookings(bookingId, supabaseAdmin);
  const recurrenceGroupId = relatedBookings[0]?.recurrence_group_id;
  const updateQuery = supabaseAdmin.from("bookings").update({
    status: "rejected",
...
export async function adminForceApprove(bookingId: string) {
  ...
  // Check for time conflicts across the whole logical request
  const relatedBookings = await getRelatedBookings(bookingId, supabaseAdmin);
  await ensureNoApprovedConflicts(relatedBookings, supabaseAdmin);

  // Approve the booking group
  const updateQuery = supabaseAdmin
    .from("bookings")
    .update({ status: "approved" });
  const recurrenceGroupId = relatedBookings[0]?.recurrence_group_id;
...
> {
  ...
  const { data: mySteps, error: stepsError } = await supabase
    .from("booking_approval_steps")
    .select(
      "booking_id, step_order, id, status, label, approver_id, decided_at, comment",
    )
  ...
  const recurringSlotsByGroup = new Map<string, RecurringBookingSlot[]>();

  if (recurrenceGroupIds.length > 0) {
    const { data: recurringBookings } = await supabaseAdmin
      .from("bookings")
      .select("id, start_time, end_time, status, recurrence_group_id")
      .in("recurrence_group_id", recurrenceGroupIds);

    for (const slot of recurringBookings || []) {
      if (!slot.recurrence_group_id) continue;
      if (!recurringSlotsByGroup.has(slot.recurrence_group_id)) {
        recurringSlotsByGroup.set(slot.recurrence_group_id, []);
      }
      recurringSlotsByGroup.get(slot.recurrence_group_id)!.push({
        id: slot.id,
  ...
  const result: ApproverBookingResult[] = mySteps
    .map((step) => {
      const b = bookings.find(
        (b: { id: string }) => b.id === step.booking_id,
      ...
      const safeBooking = {
        ...
        recurring_slots:
          typeof b.recurrence_group_id === "string"
            ? sortRecurringSlots(
                recurringSlotsByGroup.get(b.recurrence_group_id) || [],
              )
            : undefined,
      };
